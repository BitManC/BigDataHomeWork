...
public class ZooKeeper implements AutoCloseable {
     ...
     * this function is that the client path has been validated before this function is called
     *
     ...
     * with the given path. The watch will be triggered by a successful operation that deletes the node of the given
     * path or creates/delete a child under the node.
     * <p>
     *  子节点序列没有排序 同时不保证自然数和词典序
    ...
    public List<String> getChildren(final String path, Watcher watcher) throws KeeperException, InterruptedException {
        ...
        // submitRequst 是实现请求 znode 的具体方式，可以重点掌握　request 请求
        ReplyHeader r = cnxn.submitRequest(h, request, response, wcb);
        if (r.getErr() != 0) {
            throw KeeperException.create(KeeperException.Code.get(r.getErr()), clientPath);
        }
